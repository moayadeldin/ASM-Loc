...
def temporal_interpolation(temp_data, output_length):
    """
    Temporal Linear Interpolation Func.
    Args:
        temp_data ([T, dim]): the target array needed to be interpolated.
        output_length ([int / float]): temporal interpoalted coefficient.

    Returns:
        np.array [output_length, dim]: temporal intepolated arrays
    """
...
def get_segment_iou(target_segment, candidate_segment):
    ...
    tIOU = segment_intersection.astype(np.float32) / segment_union    
...
class ANETDetection(object):
    ...
    def __init__(self, ground_truth_file=None, prediction_file=None,\
                ...
                check_status=True, dataset=None, logger=None, mode='all'):
        ...
        if self.verbose:
            print_str = "\t Number of ground truth instances: {}\n".format(len(self.ground_truth)) + \
                        "\t Number of predictions: {}".format(len(self.prediction))
            print(print_str, flush=True)
            if self.logger:
                self.logger.write(print_str + '\n')
    ...
    def _import_ground_truth(self, ground_truth_file_name):
        ...
        for video_id, v in data['database'].items():
            if self.subset != 'full' and self.subset != v['subset']:
                continue
            ...
            for ann in v['annotations']:
                if ann['label'] not in activity_index:
                    ...
                    cidx += 1
                self.activity_count[ann['label']] += 1
    ...
    def _import_prediction(self, prediction_file_name):
        ...
        for video_id, v in predict_data['results'].items():
            if video_id in self.blocked_videos:
                continue
            for pred in v:
                if not pred['label'] in self.activity_index:
                    continue
    ...
    def evaluate(self):
        ...
        method.
        """
        self.ap = self.wrapper_compute_average_precision() #(9, K)
        self.mAP = self.ap.mean(axis=1)
        self.mAP = self.mAP.reshape(len(self.mAP))
        self.ap_dict = None
        if self.dataset == 'THUMOS':
            self.average_mAP = self.mAP[:7].mean()
            self.ap_dict = {}
            for class_name, class_index in self.activity_index.items():
                self.ap_dict[class_name] = self.ap[:7, class_index].mean()
            self.ap_dict = dict(sorted(self.ap_dict.items(), key=lambda item: item[1], reverse=True))
        else:
            self.average_mAP = self.mAP.mean()
        
        print_str = "-------------------------------------------------------------------------------\n" + \
                    '|t-IoU |{}|\n'.format("||".join(["{:.3f}".format(item) for item in self.tiou_thresholds])) + \
                    "-------------------------------------------------------------------------------\n" + \
                    '|mAP   |{}|\n'.format("||".join(["{:.3f}".format(item) for item in self.mAP])) + \
                    "-------------------------------------------------------------------------------\n" + \
